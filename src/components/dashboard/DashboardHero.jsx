...
export const DashboardHero = () => {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl glass-panel border border-white/10 p-4 sm:p-7 lg:p-8 bg-gradient-to-br from-emerald-950/40 via-gray-900/80 to-cyan-950/40 shadow-2xl">
      ...
      <div className="absolute top-0 right-0 -mr-24 -mt-24 w-72 sm:w-96 h-72 sm:h-96 rounded-full bg-emerald-500/15 blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 left-0 -ml-24 -mb-24 w-72 sm:w-96 h-72 sm:h-96 rounded-full bg-cyan-500/15 blur-3xl pointer-events-none" />
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6">
        ...
        <div className="space-y-3 max-w-xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-xs font-semibold">
            ...
            <span className="text-emerald-400 font-mono text-[11px]">• 100% Offline Ready</span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/[0.04] border border-white/10 text-gray-300 text-xs font-medium">
              ...
              <span>Today: <strong className="text-white font-bold">{todaySales.length} Orders</strong></span>
            </div>

            <div className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-cyan-500/10 border border-cyan-500/20 text-cyan-300 text-xs font-medium">
        ...
        <div className="w-full lg:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-4 bg-gray-950/70 p-4 sm:p-5 rounded-2xl border border-white/10 backdrop-blur-xl shadow-xl">
          <div className="pr-0 sm:pr-4 sm:border-r border-white/10 space-y-1">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-2xl sm:text-3xl font-black text-emerald-400 text-glow-green font-mono">
                +{formatCurrency(todayNetProfit, currency)}
              </span>
            </div>

            <p className="text-[11px] text-gray-400 font-mono">
              Revenue: <span className="text-cyan-400 font-semibold">{formatCurrency(todayRevenue, currency)}</span> • Cost: <span className="text-rose-400 font-semibold">{formatCurrency(todayCost, currency)}</span>
            </p>
          </div>

          <button
            onClick={() => openModal('pos')}
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl bg-gradient-to-r from-emerald-500 via-emerald-400 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-gray-950 font-black text-xs sm:text-sm shadow-glow-green hover:scale-[1.02] active:scale-[0.98] transition-all shrink-0"
          >
